...
mod tests {
    ...
    fn conversion_inversion_ecef() {
        let allowed_conversion_error = 1.00;
        ...
        for &place in [oslo, stockholm].iter() {
            ...
            close(distance, 0.0, allowed_conversion_error);
        }
    }

    #[test]
    fn conversion_ecef() {
        let allowed_conversion_error = 1.0;
    ...
    #[test]
    fn add_enu() {
        let allowed_conversion_error = 0.0001;
        ...
        for &(place, place_high) in [(oslo, oslo_high), (stockholm, stockholm_high)].into_iter() {
            let distance = ECEF::from(place_high).distance(&(ECEF::from(place) + ENU::new(0.0, 0.0, 100.0)));
            close(distance, 0.0, allowed_conversion_error);
        }
    }
    ...
    #[test]
    fn dateline() {
        // This test ensures that when moving east from the dateline
        // the longitude resets to -180
        let a = WGS84::new(20.0, 180.0, 0.0);
        let vec_a = ENU::new(10.0, 0.0, 0.0);
        let ans_a = a + vec_a;

        // This test ensures that when moving east from the dateline
        // the longitude resets to -180
        let b = WGS84::new(20.0, -180.0, 0.0);
        let vec_b = ENU::new(-10.0, 0.0, 0.0);
        let ans_b = b + vec_b;
        ...
        close(ans_a.longitude_degrees(), -180.0, 0.01);
        close(ans_b.longitude_degrees(), 180.0, 0.01);
    }
